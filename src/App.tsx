...
function createDemoState(): AppState {
  ...
  const project: Project = {
    ...
    monthlyOperatingCosts: [
      { id: generateId("cost"), amount: -1200, comment: "Vercel Pro + edge functions" },
      { id: generateId("cost"), amount: -450, comment: "Atlassian (Jira + Confluence)" },
      { id: generateId("cost"), amount: -250, comment: "Sentry + log retention" },
      { id: generateId("cost"), amount: -320, comment: "DesignOps tools (Figma/Framer)" },
    ],
    monthlyIncome: [
      { id: generateId("income"), amount: 6200, comment: "B2B subscription plans" },
      { id: generateId("income"), amount: 1800, comment: "Enterprise support add-on" },
    ],
    orderIndex: 0,
  };
  const links: LinkItem[] = [
    {
      ...
      description: "Main repository with web app, API gateway, and shared packages",
...
function normalizeImportedState(raw: unknown): AppState | null {
  ...
  const projects: Project[] = candidate.projects.map((project, index) => ({
    ...
    monthlyIncome: Array.isArray(project.monthlyIncome)
      ...
      : [],
    orderIndex: Number.isFinite(project.orderIndex) ? Number(project.orderIndex) : index,
  }));
  ...
  const links: LinkItem[] = candidate.links
    .map((link) => {
      ...
      return {
        ...
        iconCache: link.iconCache || (cleanUrl ? faviconProxy(cleanUrl) : DEFAULT_FAVICON),
      };
    })
  ...
  return {
    ...
    tags,
    selectedProjectId:
      candidate.selectedProjectId && validProjectIds.has(candidate.selectedProjectId)
...
export default function App() {
  ...
  const [financeModal, setFinanceModal] = React.useState<"expenses" | "income" | null>(
    null
  );
  ...
  const filteredLinks = React.useMemo(() => {
    ...
    if (filterTags.length) {
      list = list.filter((link) => filterTags.every((tag) => link.tags.includes(tag)));
    }
  ...
  const handleCreateProject = (data: { title: string; shortDescription: string }) => {
    setState((prev) => {
      if (!prev) return prev;
      const project: Project = {
  ...
  const handleUpdateProject = (projectId: string, data: { title: string; shortDescription: string }) => {
    setState((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
        projects: prev.projects.map((project) =>
          project.id === projectId
            ? { ...project, title: data.title, shortDescription: data.shortDescription }
            : project
        ),
      };
    });
  ...
  const handleDeleteProject = (projectId: string) => {
    setState((prev) => {
      ...
      return {
        ...prev,
        projects: projects.map((project, index) => ({ ...project, orderIndex: index })),
  ...
  const handleReorderProjectsByDrop = (sourceId: string, targetId: string) => {
    setState((prev) => {
      ...
      const sorted = [...prev.projects].sort((a, b) => a.orderIndex - b.orderIndex);
      ...
      return {
        ...prev,
        projects: sorted.map((project, index) => ({ ...project, orderIndex: index })),
      };
    });
  };
      ...
      return {
        ...
        projects: prev.projects.map((project) => {
          ...
          if (type === "expenses") {
            return {
              ...project,
              monthlyOperatingCosts: [...project.monthlyOperatingCosts, {
                id: generateId("cost"),
                amount: item.amount,
                comment: item.comment,
              }],
  ...
  const handleDeleteFinanceItem = (type: "expenses" | "income", itemId: string) => {
    if (!currentProject) return;
    setState((prev) => {
      if (!prev) return prev;
      return {
        ...
        projects: prev.projects.map((project) => {
          ...
          return {
            ...project,
            monthlyIncome: project.monthlyIncome.filter((item) => item.id !== itemId),
          };
        }),
      };
    });
  ...
  const totalExpenses = currentProject ? sumItems(currentProject.monthlyOperatingCosts) : 0;
  const totalIncome = currentProject ? sumItems(currentProject.monthlyIncome) : 0;
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[300px_1fr]">
        <aside className="border-r border-border bg-secondary/80 p-6 backdrop-blur lg:sticky lg:top-0 lg:h-screen">
          <div className="text-xs font-semibold uppercase tracking-[0.2em]">{t(locale, "brandName")}</div>
          <div className="mt-1 text-xs text-muted-foreground">{t(locale, "brandBy")}</div>

          <div className="my-4 border-t border-border" />

          <Button className="w-full justify-center" variant="outline" onClick={() => setSettingsOpen(true)}>
            {t(locale, "settings")}
          </Button>
          ...
          <div className="mt-3">
            <Combobox
              ...
              onChange={(value) =>
                setState((prev) =>
                  prev ? { ...prev, locale: value === "en" ? "en" : "ru" } : prev
                )
              }
              options={[
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isActive ? "border-border bg-background/80" : "border-transparent bg-background/40",
                ...
                >
                  <div className="flex items-start gap-2">
                    <button
                      type="button"
                      onClick={() => setState((prev) => (prev ? { ...prev, selectedProjectId: project.id } : prev))}
                      className="flex-1 text-left"
                    >
                      <div className="text-sm font-semibold">{project.title}</div>
                      <div className="text-xs text-muted-foreground">{project.shortDescription || t(locale, "noDescription")}</div>
                    </button>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setConfirmState({
                          ...
                          description: t(locale, "confirmDeleteProjectDescription"),
          ...
          {!currentProject ? (
            <Card className="max-w-xl">
              <CardContent className="p-6">
                <h2 className="text-xl font-semibold">{t(locale, "noProjectsTitle")}</h2>
                  ...
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {currentProject.shortDescription || t(locale, "noDescription")}
                      </p>
                    </div>
              ...
              <Card>
                <CardContent className="p-6">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <h2 className="text-lg font-semibold">{t(locale, "projectLinks")}</h2>
                      <p className="text-sm text-muted-foreground">
                        {t(locale, "linksCount", { filtered: filteredLinks.length, total: projectLinks.length })}
                      </p>
                    </div>
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-2">
                    <ComboboxMultiple
                      options={state.tags.map((tag) => ({ value: tag.id, label: tag.name }))}
                      value={filterTags}
      ...
      <Dialog open={financeModal !== null} onOpenChange={(open) => !open && setFinanceModal(null)}>
        <DialogContent>
            ...
            <FinanceModal
              ...
              onDelete={(itemId) =>
                setConfirmState({
                  ...
                  onConfirm: () => handleDeleteFinanceItem(financeModal, itemId),
                })
              }
    ...
    <div className="rounded-xl border border-border bg-secondary p-4 transition hover:shadow-soft">
      <a href={link.url} target="_blank" rel="noreferrer" className="block">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {tagLabels.map((label) => (
            <Badge key={`${link.id}-${label}`} variant="outline">{label}</Badge>
          ))}
        </div>
      </a>
...
function ProjectForm({
  ...
  const [description, setDescription] = React.useState(project?.shortDescription ?? "");

  return (
    <div>
      <DialogHeader>
        <DialogTitle>{project ? t(locale, "editProject") : t(locale, "newProject")}</DialogTitle>
        <DialogDescription>{t(locale, "projectNameHint")}</DialogDescription>
      </DialogHeader>

      <div className="mt-4 space-y-3">
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "projectName")}</label>
          <Input value={title} onChange={(event) => setTitle(event.target.value)} />
        </div>

        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "projectDescription")}</label>
...
function FinanceModal({
  ...
  return (
    <div>
      <DialogHeader>
        <DialogTitle>{type === "expenses" ? t(locale, "financeExpenses") : t(locale, "financeIncome")}</DialogTitle>
      </DialogHeader>

      <div className="mt-4 rounded-xl border border-border bg-muted p-4">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">
          {type === "expenses" ? t(locale, "addMonthlyExpense") : t(locale, "addMonthlyIncome")}
        </p>
        ...
        {items.map((item) => (
          ...
          >
            <div>
              ...
              <div className="text-xs text-muted-foreground">{item.comment || t(locale, "noComment")}</div>
            </div>
...
function LinkForm({
  ...
  const [selectedTags, setSelectedTags] = React.useState<string[]>(link?.tags ?? []);

  return (
    <div>
      <DialogHeader>
        <DialogTitle>{link ? t(locale, "editLinkTitle") : t(locale, "addLinkTitle")}</DialogTitle>
        <DialogDescription>{t(locale, "linkRequiredHint")}</DialogDescription>
      </DialogHeader>

      <div className="mt-4 space-y-3">
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "linkName")}</label>
          <Input value={title} onChange={(event) => setTitle(event.target.value)} />
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "linkDescription")}</label>
          <Textarea value={description} onChange={(event) => setDescription(event.target.value)} />
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "linkUrl")}</label>
          <Input value={url} onChange={(event) => setUrl(event.target.value)} />
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "linkTags")}</label>
          <ComboboxMultiple
  ...
  return (
    <div>
        ...
        {localTags.map((tag) => (
          <div key={tag.id} className="flex items-center gap-2">
            <Input
              ...
              onChange={(event) => {
                setLocalTags((prev) =>
                  prev.map((item) => (item.id === tag.id ? { ...item, name: event.target.value } : item))
                );
              }}
            />
            <Button
      ...
      <div className="mt-4 rounded-xl border border-border bg-muted p-3">
        <div className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{t(locale, "addTag")}</div>
        <div className="mt-2 flex gap-2">
          ...
          <Button
            onClick={() => {
              ...
              setLocalTags((prev) => [...prev, { id: generateId("tag"), name: cleanTag }]);
              setNewTag("");
            }}
          >
            {t(locale, "addTag")}
